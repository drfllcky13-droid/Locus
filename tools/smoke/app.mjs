...
/**
 * Generate `scans` × `points` synthetic scans, import them, start the app on the project and
 * connect over CDP. `webviewArgs` are extra WebView2 browser arguments. Returns
 * `{ cdp, child, project, close() }`; `close` kills the app and removes the temp folder.
 */
export async function launchApp({ scans = 2, points = 300_000, port = 9223, webviewArgs = "" } = {}) {
  ...
  run(validate, ["gen-scene", "--scans", String(scans), "--points-per-scan", String(points), "--out", scene]);
