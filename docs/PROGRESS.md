...
Status (2026-09-23): licence review done. Item 2 built and tested in `locus-photo`: COLMAP text-model reader (camera centres from its poses), GCP similarity (Umeyama, residuals, check points) and scale from known distances (weighted, σ the larger of the tolerances and the spread), EXIF GPS and DJI RTK tags (stdlib), WGS84 to local ENU. Items 3 onward wait on the COLMAP distribution choice and download permission (Blocked).
...
- **COLMAP distribution and downloads** (2026-09-23): Addison to choose how COLMAP reaches examiners (A: examiner-installed, path configured; B: bundled CPU-only permissive build; C: B plus a CUDA build with SiftGPU patched out and NVIDIA's runtime), approve BSL-1.0 (Boost) and public-domain SQLite if B or C, and permit downloading a COLMAP release for development and a benchmark dataset with ground truth. See `docs/phase8-colmap-licence-review.txt`.
