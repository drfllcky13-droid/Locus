...
- **Done.** Phases 0–8 are done. Phase 7's textbook-case criterion is blocked (see Open questions).
- **Phase 9 (animation), in progress.** Built so far:
  - motion model with provenance, time zero and plausibility checks;
  - timeline UI;
...
3. **Phase 11, the portable case package.** Read-only, runs from a USB drive with no install or admin rights, and shows the package hash.
4. **Phase 12, VR.** Test WebXR inside WebView2 first, and document a fallback if it's blocked.
