...
**Placing.** A mesh added to a scene gets a pivot: the bottom centre of its bounds, stored with it. Its placement (position and heading, or the move and rotate gizmo) puts that pivot at the given point, so a mesh first added sits exactly where the 3D view shows it. The placement is stored as a 4 × 4 matrix in the scene revision, with the mesh's evidence id and SHA-256. Placed meshes are drawn in renders and animations like other static objects and are exported with the scene glTF. A measured mesh is always drawn at its true size: there is no scale control.

**Exemplar models.** A mesh that stands in for something rather than measuring it (a library vehicle of the same make and model, say) can be marked as an exemplar and fitted to stated dimensions along its own x, y and z, with the source of those dimensions (a manufacturer's specification, for example). This follows common practice in reconstruction software, where library vehicle models are resized to the subject vehicle's length, width and height, while measured data is never resized. An exemplar's stated size and source are stored in the scene revision; the scene builder marks it "Resized exemplar model, not measured"; every rendered frame of a scene that has one carries "Resized exemplar model, not measured:" with its name and source, and the render is refused until a source is given.
