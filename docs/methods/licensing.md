...
Locus has three tiers, as feature flags on one program (SPEC §1):
...
- Locus holds only the publisher's public key, so a changed or forged file doesn't verify: raising the tier breaks the signature.
...
**No licence.** Without a licence file, or with one that doesn't verify, Locus runs as an **unlicensed evaluation** with every tool, and says so in the side panel and in About. Whether evaluations stay unrestricted is the publisher's decision before a commercial release (docs/ADDISON-TODO.md).
...
- **When Locus fails**, it writes a crash report to its own log folder: an unexpected error in the program (a panic, on any thread) or in its view (an uncaught exception).
- **What a report holds:** the version, the system, when it happened, the message and where in Locus's code it happened.
...
- **Nothing is sent.** On the next start Locus says a report was saved and offers to show it. The user can copy it to send to the developer themselves, or delete it.
