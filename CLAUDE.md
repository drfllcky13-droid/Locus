...
7. **Licensing.** Dependencies must be MIT, Apache-2.0, BSD, Zlib, MPL-2.0, or Unicode-3.0. Ask before adding anything GPL, AGPL, LGPL, or with unclear terms. Data-file exception: CC-BY-4.0, CC-BY-SA-3.0, LPPL and the W3C licence are allowed only for data files bundled unmodified (hyphenation patterns, citation styles, character tables), never for code. Every bundled third-party data file must have an entry in THIRD_PARTY_NOTICES.txt; CI checks this.
